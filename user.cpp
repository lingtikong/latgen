 ...
 * -------------------------------------------------------------------------- */
USER::USER(UserInput *u) : lattice(u)
{
  initialized = 0;
...
int USER::from_stdin()
{
  ...
  for (int i = 0; i < 3; ++i){
    while ( 1 ){
      ...
      if (uin->read_stdin(str) < 3) continue;
      latvec[i][0] = numeric(strtok(str,  " \t\n\r\f"));
      latvec[i][1] = numeric(strtok(NULL, " \t\n\r\f"));
      latvec[i][2] = numeric(strtok(NULL, " \t\n\r\f"));
  ...
  if (uin->read_stdin(str) > 0) nucell = inumeric(strtok(str, " \t\n\r\f"));
  if (nucell < 1) return 1;

  if (nucell != 1){
    printf("Please input the number of atom  types in cell [1]: ");
    if (uin->read_stdin(str)>0) ntype = inumeric(strtok(str, " \t\n\r\f"));
    if (ntype < 1) return 2;
